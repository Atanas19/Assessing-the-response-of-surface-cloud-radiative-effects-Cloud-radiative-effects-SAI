...
SW_WA = pd.DataFrame({'lwp':Bias_LWP_WA, 'sw':Bias_net_sw_WA})
LW_WA = pd.DataFrame({'lwp':Bias_LWP_WA, 'lw':Bias_net_lw_WA})
NET_WA = pd.DataFrame({'lwp':Bias_LWP_WA, 'net':Bias_net_WA})
...
SW_WA_df['lwp'] =SW_WA_df['lwp'] - SW_WA_df['lwp'].shift(1)
SW_WA_df['sw'] =SW_WA_df['sw'] - SW_WA_df['sw'].shift(1)

SW_WA_df =SW_WA_df.dropna()
...
train_df, test_df = splitter(SW_WA_df)
